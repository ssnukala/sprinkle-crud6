...
async function main() {
    ...
    try {
        ...
        // Generate comprehensive table of all test results
        console.log('=========================================');
        console.log('Test Results by Schema and Activity (Table Format)');
        console.log('=========================================');
        console.log('');
        
        // Collect all schemas
        const allSchemas = new Set([
            ...Object.keys(successBySchema),
            ...Object.keys(failuresBySchema)
        ]);
        ...
        if (allSchemas.size > 0) {
            ...
            for (const schema of Array.from(allSchemas).sort()) {
                const successActions = successBySchema[schema] || {};
                const failureActions = failuresBySchema[schema] || {};
