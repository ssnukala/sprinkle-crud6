...
function isValidCsrfTokens(tokens) {
    return tokens !== null && tokens !== undefined && typeof tokens.name === "string" && tokens.name.length > 0 && typeof tokens.value === "string" && tokens.value.length > 0;
}
...
async function testApiPath(page, name, pathConfig, baseUrl, csrfToken = null) {
    ...
    try {
        ...
        const isSuccess = status === expectedStatus || (status >= 200 && status < 300 && expectedStatus >= 200 && expectedStatus < 300);
...
async function takeScreenshotsFromConfig(configFile, baseUrlOverride, usernameOverride, passwordOverride) {
    ...
    try {
        ...
        page.on("request", (request) => {
            networkTracker.trackRequest(request.url(), request.method(), request.resourceType());
        });
        ...
        // SN Commenting out to reduce noise - we log all console messages above now
        ...
        */
        ...
        const selectors = ['.uk-card input[data-test="username"]', 'input[data-test="username"]', 'input[name="username"]'];
        ...
        const usernameSelectors = ['.uk-card input[data-test="username"]', 'input[data-test="username"]', 'input[name="username"]'];

        const passwordSelectors = ['.uk-card input[data-test="password"]', 'input[data-test="password"]', 'input[name="password"]', 'input[type="password"]'];
        ...
        const submitSelectors = ['.uk-card button[data-test="submit"]', 'button[data-test="submit"]', 'button[type="submit"]', '.uk-card button[type="submit"]'];
        ...
        if (Object.keys(authApiPaths).length > 0) {
            ...
            // Collect all schemas
            const allApiSchemas = new Set([...Object.keys(apiSuccessBySchema), ...Object.keys(apiFailuresBySchema)]);
            ...
            if (allApiSchemas.size > 0) {
                ...
                console.log(
                    `|-${"-".repeat(maxSchemaLen)}-|-${"-".repeat(maxActivityLen)}-|-${"-".repeat(maxResultLen)}-|-${"-".repeat(maxStatusLen)}-|-${"-".repeat(maxMessageLen)}-|`
                );
